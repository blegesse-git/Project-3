...
import { Typography } from '@material-ui/core';
  ...
  const [time, setTime] = useState( new Date().toLocaleDateString('pt-br').split( '/' ).reverse( ).join( '-' ))
  // 
  let countDown= []
...
//  console.log(countdown[0].eventDate)
 ...
 useEffect(() => {
      ...
      const timeout = setTimeout(() => {
          ...
          setTime(finalTime || new Date().toLocaleDateString('pt-br').split( '/' ).reverse( ).join( '-' ) )
